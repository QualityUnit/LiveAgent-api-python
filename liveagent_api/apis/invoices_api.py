...
class InvoicesApi(object):
    ...
    def dowload_invoice(self, invoice_number, **kwargs):
        """
        Download invoice
        Download invoice
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.dowload_invoice(invoice_number, callback=callback_function)
        ...
        if ('invoice_number' not in params) or (params['invoice_number'] is None):
            raise ValueError("Missing the required parameter `invoice_number` when calling `dowload_invoice`")
