...
class InvoicesApi(object):
    ...
    def la_api_v3_proxy_download_invoice(self, invoice_number, **kwargs):
        """
        Download invoice
        Download invoice
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.la_api_v3_proxy_download_invoice(invoice_number, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method la_api_v3_proxy_download_invoice" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        if ('invoice_number' not in params) or (params['invoice_number'] is None):
            raise ValueError("Missing the required parameter `invoice_number` when calling `la_api_v3_proxy_download_invoice`")
    ...
    def la_api_v3_proxy_get_invoices(self, **kwargs):
        """
        Invoice list
        Invoices list
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.la_api_v3_proxy_get_invoices(callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method la_api_v3_proxy_get_invoices" % key
                )
            params[key] = val
        del params['kwargs']
